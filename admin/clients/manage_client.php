...
<div class="container-fluid">
	<form action="" id="update-client">
		...
		<div class="form-group">
			...
			<small class="text-muted"><i>Để trống phần này nếu bạn không muốn cập nhật mật khẩu của mình.</i></small>
		</div>
		<div class="form-group">
			<label for="" class="control-label">Status</label>
			<select name="status" id="" class="custom-select select" required>
				<option value="1" <?php echo isset($status) && $status == "1" ? "selected" : '' ?>>Kích Hoạt</option>
				<option value="0" <?php echo isset($status) && $status == "0" ? "selected" : '' ?>>Chưa Kích Hoạt</option>
			</select>
		</div>
	</form>
</div>
